...
def show_opt(host: str, prompt: str):
    """ 控制台提示系统操作
    :param host: 谁在发起提醒
    :type host: str
    :param prompt: 提醒内容
...
show_opt("Host", "Importing modules...")
...
show_opt("Host", "Modules imported.")
...
class SpeechHandler:
    ...
    def __init__(self, model_name: str):
        ...
        # 初始化
        self.reset()
        self.workpath = os.getcwd()  # 工作路径
        self.tmp_path = os.path.join(self.workpath, "temp")  # 临时路径
    ...
    def predict(self):
        ...
        show_opt("SpeechHandler", "Recognizing speech...")
        self.info = self.pipe(self.path, batch_size=8, return_timestamps=True)["chunks"]
        show_opt("SpeechHandler", "Successfully recognized!")
    ...
    def load(self, audio_path: str, bl_pred: bool=True):
        ...
        show_opt("SpeechHandler", "Load AudioSegment.")
        ...
        if not os.path.basename(self.name) in [".mp3", ".wav", ".flac"]:
            ...
            show_opt("SpeechHandler", f"Format audio to \"{path}\".")
            try:
                self.audio.export(path, format="wav")
            except:
                show_opt("SpeechHandler", "Create temp work path.")
    ...
    def video_load(self, video_path: str, bl_pred: bool=True):
        ...
        with VideoFileClip(video_path) as video:
            show_opt("SpeechHandler", f"Extract temp .wav audio to \"{self.path}\".")
            try:
    ...
    def generate(self, slides: list[dict], ppt_path: str):
        ...
        self._raise_null()
        # 工作路径
        work_path = os.path.join(self.tmp_path, f"{self.name}_pptMaker")  # 算了，我觉得没人会同时开两个吧...
        try:
            rmtree(work_path)
        except:
            pass
        else:
            show_opt("PptGenerator", "Remove old temp folder.")
        try:
            ...
            show_opt("PptGenerator", "Create temp work path.")
        # 拷贝必要文件
        ...
        for config in slides:
            ...
            if single:
                ...
                if config["repeat"][0]:
                    ...
                    show_opt("PptGenerator", f"Save temp audio \"{name}\".")
                if config["single"]:
                    # 单个片段
        ...
        # 保存 PPT 项目
        show_opt("PptGenerator", "Saving PPT file...")
        prs.save(ppt_path)  # 保存
        show_opt("PptGenerator", f"PPT saved! At \"{ppt_path}\"")
        # 压缩整个文件夹并打包
        # show_opt("PptGenerator", "Compressing the whole ppt folder...")
        ...
        # show_opt("PptGenerator", f"PPT already exported! See at \"{zip_path}\".")
        # 切回原路径
        os.chdir(self.workpath)
        # 清理垃圾
        show_opt("PptGenerator", "Clear temp files.")
        rmtree(work_path)
...
class Window:
    ...
    class ScrollableFrame(tk.Frame):
            ...
            ---
            基于 tk.Frame 构建。  
            代码由 DeepSeek-R1 生成，经本人 Debug 并略作调整。
    ...
    class Menu(tk.Menu):
        ...
        def __init__(self, monitor, *args, **kwargs):
            ...
            self.openMenu.add_command(label="还原存档(R)", underline=5, state="disabled")
            ...
            self.exportMenu.add_command(label="保存存档(A)", underline=5, state="disabled")
            
            self.add_command(label="设置(S)", underline=3, state="disabled")
            self.add_command(label="帮助(H)", underline=3, state="disabled")
    ...
    class TopBar(tk.Frame):
        ...
        def __init__(self, monitor, *args, **kwargs):
            ...
            self.textBt.pack(side="right", padx=1)
        ...
        def _load_viewer(self):
            """ 加载全文阅览器 """
            show_opt("Window", "Deliver text viewer.")
    ...
    class SentenceBoard(tk.Frame):
        ...
        class SentenceDetail(tk.Frame):
            ...
            def highlight(self, event):
                ...
                # 选中高亮传递信息
                show_opt("SentenceBoard", f"Highlight sentence {self.key + 1}.")
            ...
            def copy(self):
                ...
                self.update()
                show_opt("SentenceBoard", f"Copy text \"{self.info["text"] if len(self.info["text"]) < 16 else self.info["text"][:16] + "..."}\"")
            ...
            def export(self, path: str):
                ...
                show_opt("SentenceBoard", f"Export audio of sentence {self.key + 1}.")
                audio.export(path, format=type[1:])

            def _ask_export(self):
                """ 询问位置并保存 """
                show_opt("SentenceBoard", "Ask for saving path.")
        ...
        class EditToplevel:
            ...
            def __init__(self, parent):
                ...
                self.parent = parent
                show_opt("EditToplevel", "Create edit toplevel.")
                self._load()
        ...
        def clear(self, key_from: int=0):
            ...
            show_opt("SentenceBoard", "Unload sentences.")
        ...
        def load(self, key_from: int=0):
            ...
            # 逐个添加
            show_opt("SentenceBoard", "Loading sentences...")
    ...
    class AudioBoard(tk.Frame):
        ...
        音频操作板
        ---
        本模块作者参考了 DeepSeek-R1 编写的代码(并非抄) ，且有部分引用。
        """

        # Part 4-1-B1. 自定义控件及类定义
        ...
        class Preview(tk.Frame):
            ...
            def load(self, audio: AudioSegment, stamp: list[float]):
                ...
                show_opt("AudioPreviewer", "Loading audio...")
            ...
            def _render(self):
                ...
                show_opt("AudioPreviewer", "Wavaform rendered.")
        ...
        class Control(tk.Frame):
            ...
            def load(self, audio: AudioSegment, stamp: list[float]):
                ...
                :param stamp: 时间戳
                :type stamp: list[float] """
                show_opt("AudioPlayer", "Loading audio...")
                # 音频信息
                self.audio = audio
                self.stamp = stamp
            ...
            def _ready_audio(self):
                ...
                show_opt("AudioPlayer", "Audio ready.")
            ...
            def _play_thread(self):
                ...
                if not self.pause_flag:
                    ...
                    show_opt("AudioPlayer", "Complete display of the whole audio.")
                    self.grid_playBt()
                # 必须停
            ...
            def play(self, bl_bt: bool=True):
                ...
                if bl_bt:
                    self.grid_pauseBt()
                    show_opt("AudioPlayer", f"User play audio. From: {self.place}s")
            ...
            def pause(self, bl_bt: bool=True):
                ...
                if bl_bt:
                    self.grid_playBt()
                    show_opt("AudioPlayer", f"User pause playing. At: {self.place}s")
            ...
            def goto(self, place: float):
                ...
                if place >= self.duration:
                    ...
                    show_opt("AudioPlayer", f"Jump to {self.place:.2f}s.")
                ...
                show_opt("AudioPlayer", f"Jump to {self.place:.2f}s. (ptr: {self.data_ptr})")
        ...
        class CutToplevel:
            ...
            def __init__(self, parent):
                ...
                show_opt("CutToplevel", "Ask for cut config.")
            ...
            def set(self):
                ...
                # 应用设置
                try:
                    text_index = self.text.index("<end>")
                    if self.text[text_index - 1] == " ":
                        text_index -= 1
                except:
                    messagebox.showwarning(title="警告", message="未能找到字符 \"<end>\" !\n请重新输入")
                else:
            ...
            def _pause_for_info(self):
                ...
                if self.parent.control.is_playing:
                    show_opt("CutToplevel", "Pause audio display.")
    ...
    class GenerateBoard:
        ...
        def ask_export(self):
            """ 询问导出位置 """
            # 询问位置
            show_opt("PptGenerator", "Ask export zipfile path. (parent folder)")
        ...
        def export(self, path: str):
            ...
            :type path: str """
            # 获取参数
            show_opt("PptGenerator", "Load user configures.")
            config = []
            for slide in self.slides:
                config += slide.get_config(), 
            # 执行导出
            show_opt("PptGenerator", "Ready to generate ppt.")
            print(config)
    ...
    def config_load(self):
        """ 基础设置加载 """
        # 配置项
        show_opt("Window", "Access json file for configs.")
    ...
    def global_activate(self):
        """ 初次加载音频激活控件 """
        show_opt("Window", "Activate widgets.")
    ...
    def open_audio(self, path: str=None):
        ...
        if path:
            ...
            show_opt("Window", f"Audio file \"{self.audio.name}\" loaded.")
            return
        # 询问文件
        show_opt("Window", "Ask audio file path.")
    ...
    def open_video(self, path: str=None):
        ...
        if path:
            self.audio.reset()
            self.audio.video_load(path, bl_pred=False)
            self.audio.predict()
            ...
            if self.first_load:
                self.first_load = False
                self.global_activate()
            show_opt("Window", f"Video file \"{self.audio.name}\" loaded.")
            return
        # 询问
        show_opt("Window", "Ask video file path.")
    ...
    def export_slice(self):
        """ 导出音频切片 """
        show_opt("Window", "Ask export sentences.")
        ...
        try:
            show_opt("Window", "Generating AudioSegment...")
            # 按需生成
            ...
            show_opt("Window", "Ask saving audio file path.")
            ...
            if path:
                ...
                show_opt("Window", f"Export audio at \"{path}\".")

    def export_txt(self):
        """ 导出全文 """
        show_opt("Window", "Ask saving .txt file path.")
        ...
        if path:
            ...
            show_opt("Window", f"Save .txt file to \"{path}\".")
            with open(path, mode="w", encoding="utf-8") as txt_file:
                txt_file.write(text)

    def generate(self):
        """ 生成 PPT 并导出 """
        show_opt("Window", "Deliver PPT Generator.")
    ...
    def __init__(self):
        ...
        self.config_load()
        # 预载管理器和模型
        show_opt("SpeechHandler", "Loading Speech-to-Text model...")
        self.audio = SpeechHandler(self.configs["model"])
        self.audio.tmp_path = self.configs["tmp_audio"]
        show_opt("SpeechHandler", f"Model \"{self.audio.model_name}\" loaded.")

        # 窗口预加载
        show_opt("Window", "Setting up root window...")
        ...
        self._set_control()
        show_opt("Window", "Packing window...")
        self._pack_control()
        show_opt("Window", "Window ready.")
        # 关闭(善良)
    ...
    def close(self):
        """ 关闭窗口 """
        show_opt("Window", "User close window.")
        show_opt("Window", "Closing matplotlib...")
        plt.close()  # 清除 matplotlib 内存占用
        if self.audio.tmp_audio:
            show_opt("Window", "Clean temp audio files...")
            self.audio.clean()  # 清理 SpeechHandler 产生的临时音频文件的存储占用
        self.root.destroy()
        show_opt("Window", "Window closed.")
...
def set_window():
    """ 创建窗口 """
    show_opt("Host", "Creating program...")
    window = Window()
    window.mainloop()
